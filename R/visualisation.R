...
plot_sample_comparison <- function(tbt_dr_res,
                                   snp_dist,
                                   sample_order = NULL, # order of sample_ids
                                   phy_pal = NULL,
                                   MAX_DIST = NULL,
                                   title = NULL) {
  # test data
  # tbt_dr_res <-
  #   readRDS('~/scratch/tbt_dr_res.rds') %>%
  #   filter(sample_id %in% c('G45160', 'G25793', '95_M2', '95_M2_RIF', 'G66296'))
  # snp_dist <- readRDS('~/scratch/snp_dist.rds')
  # sample_order = NULL
  # phylotype_colours = NULL

  # metadata <- c('sample_id')

  # need
  # 1) tbt results (just sample, phylotype, proportion, group?)
  # 2) SNP distance
  # 2) dr results (per mixture component)
