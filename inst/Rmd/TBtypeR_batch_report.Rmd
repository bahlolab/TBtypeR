...
assert_that(
  exists('tbt_dr_res') && is.data.frame(tbt_dr_res),
  exists('snp_dist') && inherits(snp_dist, 'dist'),
  exists('sample_meta') && (is.null(sample_meta) || is.data.frame(sample_meta))
)

phy_pal <- TBtypeR::tbt_palette(tbt_dr_res$mix_phylotype %>% unlist())
```

### Phylotype Distribution
```{r phy_dist}
plot_phylotype_distribution(tbt_dr_res, phy_pal = phy_pal)
```
...
plot_phylotype_heatmap(tbt_res=tbt_dr_res, snp_dist, log_scale = F)
```
