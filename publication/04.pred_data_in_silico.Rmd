...
bench_res_raw <- 
  bind_rows(
    read_tsv('04/benchmark/output/combined_results.tsv'),
    read_tsv('04/benchmark/output-var//combined_results.tsv') %>% 
...
in_silico_dataset <-  readRDS('04/in_silico_dataset.rds') 
...
saveRDS(bench_res_raw, '04/bench_res_raw.rds')
```
...
nxf_trace <-
  bind_rows(
    list.files('04/benchmark_time', pattern = 'trace-var', full.names = T) %>% 
      map_df(read_trace)  %>% 
      mutate(method = if_else(
        method == 'FastTBpipeR',
        str_c('TBtypeR (+FL)'),
        str_c(method, ' (+TBT)'))),
    list.files('04/benchmark_time', pattern = 'trace-default', full.names = T) %>% 
      map_df(read_trace) %>% 
      mutate(method = if_else(
        str_detect(method, 'TBpipeR'),
        'TBtypeR',
        method))
  ) %>% 
  filter(!is.na(method))

saveRDS(nxf_trace, '04/nxf_trace.rds')
